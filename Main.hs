...
prob11 :: Int
prob11 = maximum $ concatMap (map product . groupsOf 4 . intercalate [0,0,0]) [lr, lr_diag, ud, ud_diag]
    where
      lr      = grid
      lr_diag = filter ((>= 4) . length) $ rot45 grid
      ud      = transpose grid
      ud_diag = filter ((>= 4) . length) $ rot45 $ reverse grid
...
prob15 :: Int
prob15 = last $ go (replicate 21 1) 21
    where
      go :: [Int] -> Int -> [Int]
      go xs 1 = xs
...
prob25 :: Int
prob25 = fst $ head $ filter ((> fromInteger (10^999)) . snd) $ zip [1..] (fibs 1 1)
